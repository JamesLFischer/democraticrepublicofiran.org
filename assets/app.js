...
const prepareArticleImages = (articles) => {
  ...
  return articles.filter(article => {
    const image = article.image_url ? safeURL(article.image_url) : "";
    if (!image || image === "#" || seen.has(image)) return false;
    seen.add(image);
    article._displayImage = image;
    return true;
  });
};
...
const visual = (a, size = "card") => {
  ...
  return `
    <div class="visual ${publisherImage ? "has-image" : "no-image"}" aria-hidden="true">
      ${publisherImage ? `<img class="publisher-art" src="${publisherImage}" alt="" loading="${size === "lead" ? "eager" : "lazy"}" decoding="async" referrerpolicy="no-referrer">` : ""}
