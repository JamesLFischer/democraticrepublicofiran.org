...
function prepareArticles(rawArticles) {
  ...
  for (const article of rawArticles) {
    ...
    if (!article || !article.title || url === "#" || image === "#") continue;
    if (seenImages.has(image) || seenIds.has(article.id)) continue;
...
function setNavAvailability() {
  document.querySelectorAll(".nav-link").forEach(button => {
    ...
    button.classList.toggle("has-no-stories", count === 0);
...
/* The old footer had #year. V19 intentionally removed it.
   Guarding this prevents the entire feed from crashing. */
